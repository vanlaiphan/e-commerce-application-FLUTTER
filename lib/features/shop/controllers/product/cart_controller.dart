...
class CartController extends GetxController {
  ...
  void addToCart(ProductModel product) {
    ...
    int index = cartItems
        .indexWhere((cartItems) => cartItems.productId == selectedCartItem.productId && cartItems.variationId == selectedCartItem.variationId);

    if (index >= 0) {
      // This quantity is already add or Update/Removed from the design (Cart)(-)
  ...
  void removeFromCartDialog(int index) {
    Get.defaultDialog(
      ...
      onConfirm: () {
        ...
        updateCart();
        TLoaders.customToast(message: 'Product remove from the Cart.');
        Get.back();
      },
      onCancel: () => () => Get.back(),
    );
